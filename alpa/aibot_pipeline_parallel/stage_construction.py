...
from alpa.pipeline_parallel.stage_profiling import (
    get_compute_cost, last_compute_cost_file_name)
from alpa.shard_parallel.auto_sharding import AutoShardingOption
from alpa.timer import timers
from alpa.util import OrderedSet, jaxpr_to_hlo, maybe_numba_jit
...
def cluster_layers_and_slice_mesh(
        ...
        default_as_option: AutoShardingOption, stage_option: StageOption, gensym_func):
    """
    ...
    if not inference_mode:
        ...
        # cygutil.setCriticalNodeColor(draw_DAG, [var for var in critical_node if var not in ['s','e']])
        # draw_DAG = nx.nx_pydot.to_pydot(draw_DAG)
        # draw_DAG.write("cyg_test/display/results/test_jaxpr_dag_with_pipeline_marker.png",format="png")
    
        
        # 按照关键路径，进行layer层内进行划分
        sliced_critical_layers = []
        sliced_parallel_layers = []
        left = 0
        right = 0
        for i, jax_pipeline_layer in enumerate(back_layers):
            ...
            # print(np.matmul(partition,weight))
            
            left = right
        
        sliced_critical_layers.extend(sliced_parallel_layers)
        sliced_layers = sliced_critical_layers
        # # 更新每个公式的floats，暂时设置为1
        # for jpc in sliced_layers:
        #     eqn_index_to_floats = []
        #     for eqn_index,eqn in enumerate(jpc.closed_jaxpr().jaxpr.eqns):
        #         # eqn_index_to_floats[eqn_index] = 1
        #         eqn_index_to_floats.append(cygutil.eqn_flops(eqn))
        #     print(np.sum(np.array([eqn_index_to_floats])))
            
        # sliced_jaxpr = cygutil.transJaxPipelineComputationsToMergedJaxpr(sliced_layers)
        # sliced_dag_with_var = cygutil.translateJaxprToNetworkxWithoutVar(sliced_jaxpr)
        # sliced_dag_without_var = cygutil.translateJaxprToNetworkxWithoutVarAndPipelineMarker(sliced_jaxpr)
        # sliced_dag_with_var = nx.nx_pydot.to_pydot(sliced_dag_with_var)
        # sliced_dag_without_var = nx.nx_pydot.to_pydot(sliced_dag_without_var)
        # sliced_dag_with_var.write("cyg_test/display/results/test_sliced_dag_with_pipeline.png",format="png")
        # sliced_dag_without_var.write("cyg_test/display/results/test_sliced_dag_without_var.png",format="png")
        
        layers = layers[:start] + sliced_layers
        
        # 获得更新后的 stage_layer_ids, stage_layer_mesh
        backward_parallel_stage_layer_ids = [[
            3 * num_layers - 1 - i for i in reversed(layer_ids)
        ] for layer_ids in reversed(forward_stage_layer_ids)]
        stage_layer_ids = forward_stage_layer_ids + backward_stage_layer_ids + backward_parallel_stage_layer_ids
        stage_to_mesh = list(range(num_forward_stages)) + list(
            reversed(range(num_forward_stages))) + list(
            reversed(range(num_forward_stages)))
...
def get_stage_outvars(layers: Sequence[JaxPipelineComputation],
    ...
    # # 崔勇敢修改代码 开始
    # # 原始的代码只能针对顺序划分，下述代码将针对所有的划分
    # n_stages = len(layer_assignment)
    # used = [OrderedSet(global_outvars) for _ in range(n_stages)]
    # stage_outvars = [OrderedSet() for _ in range(n_stages)]

    # for stage_id, layer_ids in enumerate(layer_assignment):
    #     used_update_index=[i for i in range(n_stages) if i != stage_id ]
    #     for layer_id in layer_ids:
    #         for var in layers[layer_id].invars:
    #             for i in used_update_index:
    #                 used[i].add(var)

    # for stage_id, layer_ids in reversed(list(enumerate(layer_assignment))):
    #     for layer_id in layer_ids:
    #         for var in layers[layer_id].outvars:
    #             if var in used[stage_id]:
    #                 stage_outvars[stage_id].add(var)
    # return stage_outvars
    # # 崔勇敢修改代码 结束
