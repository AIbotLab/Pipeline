...
from alpa import (AutoLayerOption, AutoShardingOption, AutoStageOption,
                  BppPipeshardParallel, ManualStageOption, PhysicalDeviceMesh,
                  PipeshardParallel, ShardParallel)
...
def write_csv_row(csv_file_path:str,write_datas:list):
    csv_file = open(csv_file_path, "a", newline='')
    csv_writer = csv.writer(csv_file,delimiter='\t')
    csv_writer.writerow(write_datas)
    csv_file.flush()
    csv_file.close()



def get_bpp_pipeshard_parallel_method(benchmark_case: BenchmarkCase,
...
def compile_and_benchmark_pipeshard_training_executable(
    ...
    for i in range(1,repeat_num+1):
        latencie,state = benchmark_training_executable(
            niter,
            train_step,
            executable,
            state,
            other_train_step_inputs,)
    latencies.extend(latencie)
