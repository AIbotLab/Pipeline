...
from cyg_test.experiments.experiment_util import (
    ...
    write_csv_row)
from cyg_test.experiments.moe.alpa.moe_benchmarks_config import (
    alpa_moe_best_suite, alpa_moe_search_suite)
from cyg_test.experiments.moe.alpa.moe_util import (
    compute_moe_statistics, get_train_step, prepare_moe_input_and_model)

moe_suites={
    "best":alpa_moe_best_suite,
    "search":alpa_moe_search_suite
}
...
def benchmark_all(args):
    ...
    for benchmark_case in moe_suites[suite][num_gpus]:
        ...
        values = [model, method_name, enable_overlapping,seq_len, hidden_size, num_layers, num_heads,vocab_size, num_expert, expert_group_size,
        ...
        for i in range(1,2):
            ...
            p = ctx.Process(target=benchmark_and_write_to_namespace,
                            args=(result_namespace, benchmark_case,niter,repeat_num,num_hosts,num_devices_per_host,method_name,disable_tqdm),
                            )
            p.start()
            p.join()
            if p.exitcode != 0:
            ...
            if i == 1:
                if parallel_mode == "load_solution":
                    ...
                    exit()
                total_flop, parameter_count = compute_moe_statistics(benchmark_case, [0], num_gpus)
