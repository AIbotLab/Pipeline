...
from cyg_test.experiments.experiment_util import (
    ...
    write_csv_row)
from cyg_test.experiments.wresnet.alpa.wresnet_benchmarks_config import (
    alpa_wresnet_best_suite, alpa_wresnet_search_suite)
from cyg_test.experiments.wresnet.alpa.wresnet_util import (
    compute_param_number, get_train_step, prepare_wresnet_input_and_model)

wresnet_suites={
    "best":alpa_wresnet_best_suite,
    "search":alpa_wresnet_search_suite
}
...
def benchmark_one_case(model,
    ...
    total_flop = executable.flop_count
    ...
    return max_mem_allocated, latencies, metadata, total_flop, parameter_count
...
def benchmark_all(args):
    ...
    try:
        _ = wresnet_suites[suite][num_gpus]
    except KeyError:
        print(f"No available benchmark suite for {args.suite} with {num_gpus} GPUs.",flush=True)
        exit()

    for benchmark_case in wresnet_suites[suite][num_gpus]:
        ...
        values = [model, method_name, enable_overlapping, image_size, num_layers, num_channels, width_factor, dtype, 
        ...
        for i in range(1,2):
            ...
            p = ctx.Process(target=benchmark_and_write_to_namespace,
                            args=(result_namespace, model, benchmark_case,niter,repeat_num,num_hosts,num_devices_per_host,method_name,disable_tqdm),
                            )
            p.start()
            p.join()
            if p.exitcode != 0:
                total_flop, parameter_count, peak_mem, latencies, metadata = np.inf, np.inf, np.inf, [np.inf], {"forward_stage_layer_ids": [],
                                                                "submesh_shapes": [],
                                                                "logical_mesh_shapes": [],
                                                                "autosharding_option_dicts": [],}
            else:
                peak_mem, latencies, metadata, total_flop, parameter_count =  result_namespace.result
            if i == 1:
                if parallel_mode == "load_solution":
                    ...
                    exit()
                values.extend([parameter_count/1e9,total_flop/1e12, peak_mem/GB])
